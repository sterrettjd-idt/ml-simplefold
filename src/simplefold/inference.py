...
def initialize_folding_model(args):
    ...
    if not os.path.exists(ckpt_path):
        ...
        os.system(f"curl -L {ckpt_url_dict[simplefold_model]} -o {ckpt_path}/{simplefold_model}.ckpt")
...
def initialize_plddt_module(args, device):
    ...
    if not os.path.exists(plddt_ckpt_path):
        ...
        os.system(f"curl -L {plddt_ckpt_url} -o {plddt_ckpt_path}/plddt.ckpt")
    ...
    if not os.path.exists(plddt_latent_ckpt_path):
        os.makedirs(args.ckpt_dir, exist_ok=True)
        os.system(f"curl -L {ckpt_url_dict['simplefold_1.6B']} {plddt_latent_ckpt_path}/simplefold_1.6B.ckpt")
